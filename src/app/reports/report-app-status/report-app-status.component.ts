...
@Component({
  selector: 'app-report-app-status',
  templateUrl: './report-app-status.component.html',
  styleUrls: ['./report-app-status.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportAppStatusComponent implements OnInit {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    ...
    this.orderService.getReports('app-status', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'appStatus';
